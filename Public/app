document.addEventListener('DOMContentLoaded', ()=>{
 ...
 let currentPlayer = 'user';
  ...
  // Selecione o modo de jogador

  singlePlayerButton.addEventListener('click', startSinglePlayer); // contra o pc
  multiPlayerButton.addEventListener('click', startMutliPlayer); //contra outro jogador
  ...
  // Obter seu número de jogador (Player 1 ou Player 2)
  socket.on('player-number', (num)=>{
    if(num === -1){
      ...
      // Obter status de outros jogadores
      socket.emit('check-players'); //checando
      
    }
  })

  // Outro jogador se conectou ou desconectou
  ...
  // Desconectar 
  socket.on('timeout', ()=>{
    infoDisplay.innerHTML = 'Você atingiu o limite de 10 minutos';
  })
  ...
  // Demarcar as embarcações do adversário 
